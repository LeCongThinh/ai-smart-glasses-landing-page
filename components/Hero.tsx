...
export default function Hero() {
    return (
        <section className="relative overflow-hidden bg-[#faf8ff] px-5 pt-0 pb-12 text-[#131b2e] md:px-16 lg:min-h-[calc(100vh-88px)] lg:pt-2 lg:pb-16">
            {/* Background glow */}
            ...
            <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:gap-12">
                ...
                <div className="text-left lg:text-left">
                    <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/80 px-5 py-2 text-sm font-semibold text-[#2563eb] shadow-[0_20px_50px_rgba(37,99,235,0.08)] backdrop-blur-2xl">
                        ✦ Next Generation AI Smart Glasses
                    </div>

                    <h1 className="mt-8 max-w-2xl text-[48px] font-bold leading-[1.05] tracking-[-0.04em] text-[#131b2e] md:text-[64px] lg:text-[78px]">
                        See <span className="text-[#2563eb]">Smarter.</span>
                        <br />
                        Live <span className="text-[#2563eb]">Better.</span>
                    </h1>

                    <p className="mt-6 max-w-xl text-lg leading-[1.6] text-[#434655]">
                    ...
                    <div className="mt-10 flex flex-col gap-4 sm:flex-row">
                        <a href="#pre-order" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#2563eb] 
                        px-8 py-4 font-semibold text-white shadow-[0_20px_50px_rgba(37,99,235,0.22)] transition-all hover:-translate-y-0.5 
                        hover:shadow-[0_25px_70px_rgba(37,99,235,0.32)]"> Pre-order Now <ArrowRight size={18} />
                        </a>

                        <a href="#demo" className="inline-flex items-center justify-center gap-2 rounded-full border border-[#2563eb]/40
                         bg-white/60 px-8 py-4 font-semibold text-[#2563eb] shadow-[0_20px_50px_rgba(37,99,235,0.08)] backdrop-blur-2xl 
                         transition-all hover:-translate-y-0.5 hover:bg-white"> Watch Demo <Play size={18} />
                        </a>
                    </div>
                ...
                {/* RIGHT SHOWCASE */}
                <div className="relative order-2 mt-8 flex min-h-90 items-center justify-center overflow-hidden lg:order-0 lg:mt-0 lg:min-h-162.5">
                    {/* Background glow */}
...
function FeatureCard({ icon, title, desc, className, }: { icon: React.ReactNode; title: string; desc: string; className?: string; }) {
    return (
        <div className={`absolute z-40 flex items-center gap-2 rounded-xl border border-white/70 bg-white/40 px-3 py-2 shadow-[0_10px_25px_rgba(37,99,235,0.1)] backdrop-blur-xl lg:gap-4 lg:rounded-3xl lg:px-5 lg:py-4 ${className}`}>
            ...
            <div className="leading-tight">
                <p className="text-[11px] font-semibold text-[#131b2e] lg:text-base">{title}</p>
                <p className="text-[10px] text-[#434655] lg:text-sm">{desc}</p>
            </div>
        </div>
    );
}
...
function Stat({ icon, title, desc, }: { icon: React.ReactNode; title: string; desc: string; }) {
    return (
        <div className="flex flex-col items-center rounded-2xl border border-white/70 bg-white/60 p-3 text-center 
        shadow-[0_10px_30px_rgba(37,99,235,0.08)] backdrop-blur-xl lg:flex-row lg:items-center lg:gap-3 lg:p-4 lg:text-left">
            ...
            <div>
                <p className="text-xs font-bold text-[#131b2e] lg:text-sm">{title}</p>
                <p className="text-[10px] text-[#434655] lg:text-xs">{desc}</p>
            </div>
        </div>
    );
}